...
//#include <wiiuse/wpad.h> //TODO: Port to WiiU libs
...
static int I_TranslateKey(SDL_Keysym* key) //Fixed?
{
  int rc = 0;
  ...
  */
  ...
  case SDLK_LALT:
  //case SDLK_LMETA:  //TODO: Check for WiiU
  case SDLK_RALT:
  //case SDLK_RMETA:  rc = KEYD_RALT;   break;  //TODO: Check for WiiU
...
static void I_UploadNewPalette(int pal)
{
  ...
  //SDL_SetPalette(SDL_GetVideoSurface(),SDL_PHYSPAL,colours+256*pal, 0, 256); //TODO: Port to WiiU libs
}
...
void I_FinishUpdate (void)
{
  ...
  SDL_UpdateWindowSurface(screen);  //FIXED ?
}
...
{
  /* //Fixed ? I dont think this is necesary the resolution will be fixed for this WiiU port
...
//  int iteration;
  ...
  //modes = SDL_ListModes(NULL, flags); // FIXED ?
  int16_t modes[2] = {1, 2};
...
// It affects only fullscreen modes.
  if (V_GetMode() == VID_MODEGL) {
    ...
    {
      I_ClosestResolution(&width, &height, SDL_WINDOW_FULLSCREEN | SDL_WINDOW_OPENGL);
    }
    SCREENWIDTH = width;
    SCREENHEIGHT = height;
...
void I_InitGraphics(void)
{
  ...
  if (firsttime)
  {
    ...
    /* Set the video mode */
    //I_UpdateVideoMode(); FIXME Remplace with WiiU seet video Mode
...
#ifndef _DEBUG
    //init_flags |= SDL_HWPALETTE;  //TODO: Port to WiiU libs
#endif
  }
